...
// Migrations take an advisory lock, which fails through Neon's pooler (P1002) — use a direct connection
const direct = ["DATABASE_URL_UNPOOLED", "POSTGRES_URL_NON_POOLING"].map((k) => process.env[k]?.trim()).find(Boolean);
execSync("npx prisma migrate deploy", { stdio: "inherit", env: direct ? { ...env, DATABASE_URL: direct } : env });
